...
export default function CreatePollModal({
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof FormData, string>> & { submit?: string }
  >({});
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.startDate) {
      ...
      newErrors.startDate =
        "A data de início deve ser pelo menos 1 minuto no futuro";
    }
    ...
    const validOptions = formData.voteOptions.filter(
    ...
    if (
      formData.choiceLimitPerUser < 1 ||
      formData.choiceLimitPerUser > validOptions.length
    ) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label
            ...
            className="block text-sm font-medium mb-1"
          >
            Limite de Escolhas por Usuário
          </label>
          <Input
            id="choiceLimit"
            type="number"
            ...
            onChange={(e) =>
              setFormData((prev) => ({
                ...prev,
                choiceLimitPerUser: parseInt(e.target.value),
              }))
            }
          />
          {errors.choiceLimitPerUser && (
        ...
        <div>
          ...
          {errors.voteOptions && (
            <p className="text-red-500 text-sm mt-1">{errors.voteOptions}</p>
          )}
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          <Button type="submit" scheme="primary" disabled={isSubmitting}>
