...
public final class PollMapper implements Mapper<Poll, PollEntity> {
    ...
    @Override
    public Poll parseToModel(PollEntity pollEntity) {
        return Poll.parseUnsafe(
                ...
                pollEntity.getVoteOptions().stream().map(voteOptionMapper::parseToModel).toList(),
    ...
    @Override
    public PollEntity parseToEntity(Poll poll) {
        return PollEntity.builder()
                ...
                .voteOptions(poll.getVoteOptions().stream().map(voteOptionMapper::parseToEntity).toList())
